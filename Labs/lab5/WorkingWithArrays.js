let todos = [
    { id: 1, title: "Task1", completed: false, description: "NodeJS server with ExpressJS" },
    { id: 2, title: "Task2", completed: true, description: "NodeJS server with ExpressJS" },
    { id: 3, title: "Task3", completed: false, description: "NodeJS server with ExpressJS" },
    { id: 4, title: "Task4", completed: true, description: "NodeJS server with ExpressJS" },
];

export default function WorkingWithArrays(app) {
    const getTodos = (req, res) => {
        const { completed } = req.query;
        if (completed !== undefined) {
            const isCompleted = completed === "true";
            return res.json(todos.filter(t => t.completed === isCompleted));
        }
        res.json(todos);
    };
    const getTodosById = (req, res) => {
        const { id } = req.params;
        res.json(todos.find(t => t.id === parseInt(id)));
    };
    const createNewTodo = (req, res) => {
        const newTodo = {
            id: new Date().getTime(),
            title: "New Task",
            completed: false,
            description: ""
        };
        todos.push(newTodo);
        res.json(todos);
    };
    const postNewTodo = (req, res) => {
        const todo = req.body;

        const newTodo = {
            id: new Date().getTime(),
            title: todo.title || "New Task",
            description: todo.description || "",
            completed: todo.completed ?? false,
        };
        todos.push(newTodo);
        res.json(todos);
    };
    const removeTodo = (req, res) => {
        const { id } = req.params;
        todos = todos.filter(t => t.id !== parseInt(id));
        res.json(todos);
    };
    const updateTodoTitle = (req, res) => {
        const { id, title } = req.params;
        const todo = todos.find(t => t.id === parseInt(id));
        todo.title = title;
        res.json(todos);
    };
    const updateTodoDescription = (req, res) => {
        const { id, description } = req.params;
        const todo = todos.find(t => t.id === parseInt(id));
        todo.description = description;
        res.json(todos);
    };
    const updateCompleted = (req, res) => {
        const { id, completed } = req.params;
        const todo = todos.find((t) => t.id === parseInt(id));
        todo.completed = completed === "true";
        res.json(todos);
    };
    const deleteTodo = (req, res) => {
        const { id } = req.params;
        const todoIndex = todos.findIndex((t) => t.id === parseInt(id));
        if (todoIndex === -1) {
            res.status(404).json({ message: `Unable to delete Todo with ID ${id}` });
            return;
        }
        todos.splice(todoIndex, 1);
        res.sendStatus(200);
    };
    const updateTodo = (req, res) => {
    const { id } = req.params;
    const todoIndex = todos.findIndex((t) => t.id === parseInt(id));
    if (todoIndex === -1) {
      res.status(404).json({ message: `Unable to update Todo with ID ${id}` });
      return;
    }
    todos = todos.map((t) => {
      if (t.id === parseInt(id)) {
        return { ...t, ...req.body };
      }
      return t;
    });
    res.sendStatus(200);
  };
  app.put("/lab5/todos/:id", updateTodo);
    app.delete("/lab5/todos/:id", deleteTodo);
    app.get("/lab5/todos", getTodos);
    app.get("/lab5/todos/create", createNewTodo); 
    app.post("/lab5/todos", postNewTodo);       
    app.get("/lab5/todos/:id", getTodosById);
    app.get("/lab5/todos/:id/delete", removeTodo);
    app.get("/lab5/todos/:id/title/:title", updateTodoTitle);
    app.get("/lab5/todos/:id/description/:description", updateTodoDescription);
    app.get("/lab5/todos/:id/completed/:completed", updateCompleted);
}
